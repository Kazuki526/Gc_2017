...
while(<IN>){
		...
		#system("trimmomatic PE -phred33 $fastaq1 $fastaq2 $pairfaq1 /dev/null $pairfaq2 /dev/null ILLUMINACLIP:$seqdir/adaptar.fa:2:20:10 SLIDINGWINDOW:4:25 LEADING:20 TRAILING:20 MINLEN:30");
		my($replibam,$identbam)=("$replicatedir/$sample"."to_T_aes_Cluster_AllTranscripts.bam","$ABDidenticaldir/$sample"."to_ABD_identical.bam");
